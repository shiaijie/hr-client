...
/*  getCaptcha() {
  ...
  submit() {
    ...
    this.http.post('http://localhost:9090/user/login?_allow_anonymous=true', {
      jobCode: this.jobCode.value,
      password: this.password.value,
    }).subscribe((res: any) => {
      if (!res.meta.success || res.data === null) {
        this.error = res.meta.message;
        this.notifcication.error("账号或密码错误");
        return;
      }
      sessionStorage.setItem(environment.SESSIONNAME, JSON.stringify(res.data));
      this.router.navigate(['roster']);
    });
